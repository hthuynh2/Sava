...
string Protocol::create_I_msg(vector<string> vm_list, int vm_num_){
    ...
    if((master1_id == -1 || master1_id == vm_num_) && master_id == my_vm_info.vm_num){
        msg += int_to_string(vm_num_);
        msg += "99";
        master1_id = vm_num_;
        cout << "master1_id = " << master1_id;
    }
    else if((master2_id == -1 && master1_id != vm_num_)&& master_id == my_vm_info.vm_num){
        msg += int_to_string(master1_id);
        msg += int_to_string(vm_num_);
        master2_id = vm_num_;
        cout << "master2_id = " << master2_id;
    }
    else {
        msg += "99";
        msg += "99";
...
void Protocol::handle_J_msg(string msg){
    ...
    if(is_in_ML == false){
        ...
        //NEW??
        port_map[new_vm.ip_addr_str] = new_vm.vm_num;
...
//    string create_MU_msg(int failed_node, int master1_id, int master2_id){
    if(i_msg.substr(i_msg.size()-3) != "99\n" && my_vm_info.vm_num == master_id){
        cout << "Check 1: " << i_msg.substr(i_msg.size() -3)<<"\n";
        string mu_msg = create_MU_msg(99, master1_id, master2_id);
        int master1_sock_fd = tcp_open_connection(vm_info_map[master1_id].ip_addr_str, STABILIZATION_PORT );
        if(master1_sock_fd != -1){
            int numbytes = tcp_send_string(master1_sock_fd, mu_msg);
            cout << "sent MU msg to master1: "<< mu_msg<<"\n";
        }
    }
...
void Protocol::handle_I_msg(string msg){
    ...
    //MP3: extract new master id
    int new_master = string_to_int(msg.substr(msg.size()-7 ,2 ));
    cout << "Inside handle_I_msg : "  << "new master id = " << new_master;
    cout << msg.substr(msg.size()-4);
    
    int new_master1 = string_to_int(msg.substr(msg.size()-5 ,2 ));
    int new_master2 = string_to_int(msg.substr(msg.size()-3 ,2 ));
...
//    if(msg[msg.size() -2] == '1'){
//        master1_id = my_vm_info.vm_num ;
//    }
//    else if(msg[msg.size() -2] == '2'){
//        master2_id = my_vm_info.vm_num ;
//    }
    master_lock.unlock();
    ...
    for(int i = 0 ; i < (int)nodes.size(); i++){
        ...
        //NEW??
        port_map[new_node.ip_addr_str] = new_node.vm_num;
    }
...
void Protocol::handle_G_msg(string msg, bool haveLock) {
    ...
    if(str[0] == 'N'){
        ...
        if(membership_list.find(target_id) != membership_list.end()){
            ...
            cout << "Inside gossip: "<< str;
            // string new_node_num_str = msg.substr(1,2);
            // int new_node_num = string_to_int(new_node_num_str);
            failed_node =  string_to_int(str.substr(1,2));
            cout << "failed node = " << failed_node <<"\n";
        }
        else{
            if(haveLock == false)
                membership_list_lock.unlock();
    ...
    if(flag_need_to_fix==true && failed_node != -1){
        //MP3!!
        cout << "failed node = " << failed_node <<"\n";
        thread node_fail_thread = std::thread(node_fail_handler, failed_node);
